...
public class ShootingAppActivity 
    ...
    private void initSensors() 
        ...
        rawAcclSensor = sensorManager.getDefaultSensor(Sensor.TYPE_ACCELEROMETER);
        geoMagSensor = sensorManager.getDefaultSensor(Sensor.TYPE_MAGNETIC_FIELD);
        if( acclSensor == null ) {
        ...
        }else if( rawAcclSensor == null ) {
            throw new Exception("Oops, there is no linear accelerometer on this device :(");
        }else if( geoMagSensor == null ) {
            throw new Exception("Oops, there is no geomagnetic sensor on this device :(");
        }
    }
    ...
    private void startSensing() {
        ...
        sensorManager.registerListener( this ,                              // Listener
                                        ...
                                        SensorManager.SENSOR_DELAY_GAME );  // Measurement interval (microsec)
        sensorManager.registerListener(this, rawAcclSensor, SensorManager.SENSOR_DELAY_GAME); // one line pls
        sensorManager.registerListener(this, geoMagSensor, SensorManager.SENSOR_DELAY_GAME); // one line pls
    }
    ...
    public void onSensorChanged( SensorEvent event ) {
        ...
        // Case 3: Rotation sensor
        else if(event.sensor.getType() == Sensor.TYPE_ACCELEROMETER){
            acclMatrix = event.values.clone();
            processRotValues();
    ...
    /** Processes rotation vector values. */
    private void processRotValues(){
        float [] rotationMatrix= new float[9];
        float [] remappedRotationMatrix = new float[9];
        float [] headingValues = new float[3];
        SensorManager.getRotationMatrix(rotationMatrix, null ,acclMatrix, magMatrix);
        ...
        SensorManager.getOrientation(remappedRotationMatrix, headingValues);
        double degrees = headingValues[0] * 180 / Math.PI;
        if (degrees < 0) {
            degrees = 360 + degrees;
        }
        shootingDirection = (float)degrees;
    ...
    /**Rotation sensor. */
    private Sensor geoMagSensor;
    private Sensor rawAcclSensor;
    ...
    private float acclMatrix[] = new float[3];
